...
use bpmn_lite_server::grpc::proto::{
    ...
    HealthRequest, InspectRequest, RegisterGrpcTemplateRequest, StartRequest,
};
...
async fn connect(url: &str) -> Result<BpmnLiteClient<Channel>> {
    BpmnLiteClient::connect(url.to_string()).await.with_context(|| format!("connect to {}", url))
}
...
async fn poll_completed(client: &mut BpmnLiteClient<Channel>, id: &str, timeout: Duration) -> Result<()> {
    let deadline = Instant::now() + timeout;
    loop {
        let state = client.inspect(InspectRequest { process_instance_id: id.to_string(), tenant_id: String::new() })
        ...
        if Instant::now() > deadline { bail!("{} not COMPLETED in {}s", id, timeout.as_secs()); }
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
}

async fn run_case(client: &mut BpmnLiteClient<Channel>, bytecode_version: Vec<u8>, client_id: &str, label: &str) -> Result<()> {
    let payload = "{}";
    ...
    let (job_key, claim_token, current_hash) = loop {
        ...
        if Instant::now() > deadline { bail!("seed_data job not available for {}", instance_id); }
        tokio::time::sleep(Duration::from_millis(50)).await;
    };

    let domain_payload = serde_json::json!({ "do_client_id": client_id }).to_string();
    client.complete_job(CompleteJobRequest {
        job_key, domain_payload,
        domain_payload_hash: current_hash,
        orch_flags: HashMap::new(),
        worker_id: "grpc_proof".to_string(),
        claim_token, tenant_id: String::new(),
    }).await?;

    poll_completed(client, &instance_id, Duration::from_secs(15)).await.with_context(|| format!("case {}", label))?;
...
async fn main() -> Result<()> {
    ...
    let grpc_target_url = args.windows(2).find(|w| w[0] == "--grpc-target-url").map(|w| w[1].clone())
        .unwrap_or_else(|| "http://127.0.0.1:50099".to_string());

    println!("B-gRPC FFI proof — server: {}  grpc-target: {}", server_url, grpc_target_url);
    ...
    let reg = client.register_grpc_template(RegisterGrpcTemplateRequest {
    ...
    run_case(&mut client, bytecode_version.clone(), "ACME", "ACME approved").await?;
