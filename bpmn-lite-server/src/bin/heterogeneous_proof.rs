//! B9 — Heterogeneous FFI proof: HTTP + dmn-lite in one BPMN process.
//!
//! This is the V&S v1.1 Claim 2 proof: the bpmn-lite FFI is genuinely generic.
//!
//! Process:
//!   Start
...
//!     → XOR gateway (= do_eligible == true → eligible_end / → denied_end)
//!
//! Two cases:
//!   ACME  → HTTP score=720 → dmn-lite score in [700..*] → eligible=true → eligible_end COMPLETED
//!   REJECT → HTTP score=450 → dmn-lite catch-all → eligible=false → denied_end COMPLETED
//!
//! V&S Claim 2: two heterogeneous vocabularies, one process, one Flag (do_score) as the bridge.
//!
//! Usage:
...
use bpmn_lite_server::grpc::proto::{
    ...
    HealthRequest, InspectRequest, RegisterDmnDecisionRequest, RegisterHttpTemplateRequest,
    StartRequest,
};
...
/// Build the heterogeneous BPMN process XML.
///
/// Data flow:
///   do_client_id (string, DomainPayload) → HTTP input
///   do_score     (integer, Flag)         → HTTP output → dmn-lite input
///   do_eligible  (bool, Flag)            → dmn-lite output → gateway condition
fn build_bpmn_xml(http_template_id: &str, dmn_template_id: &str) -> String {
    format!(
...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL">
  <bpmn:process id="heterogeneous_proof" isExecutable="true">
    ...
    <bpmn:dataObject id="do_score">
      <bpmn:extensionElements>
        <!-- integer → Flag storage: HTTP writes it, dmn-lite reads it -->
    ...
    <!-- HTTP FFI: POST /credit-check → score field → do_score Flag -->
    ...
    <!-- dmn-lite FFI: reads do_score Flag, writes do_eligible Flag -->
    ...
    <bpmn:sequenceFlow id="f4" sourceRef="eligibility_decision" targetRef="gw"/>
    <bpmn:sequenceFlow id="f5" sourceRef="gw" targetRef="eligible_end">
      <bpmn:conditionExpression>= do_eligible == true</bpmn:conditionExpression>
    </bpmn:sequenceFlow>
    <bpmn:sequenceFlow id="f6" sourceRef="gw" targetRef="denied_end"/>
  </bpmn:process>
</bpmn:definitions>"#,
    ...
    loop {
        if let Ok(mut c) = connect(server_url).await {
            if let Ok(r) = c.health(HealthRequest {}).await {
                if r.into_inner().ready { return Ok(()); }
            }
        }
        if Instant::now() > deadline { bail!("server not ready after {}s", timeout.as_secs()); }
...
async fn poll_completed(client: &mut BpmnLiteClient<Channel>, instance_id: &str, timeout: Duration) -> Result<()> {
    let deadline = Instant::now() + timeout;
    loop {
        let state = client.inspect(InspectRequest {
            process_instance_id: instance_id.to_string(),
            tenant_id: String::new(),
        }).await?.into_inner().state;
        match state.as_str() {
            "COMPLETED" => return Ok(()),
            "RUNNING" => {}
            other => bail!("unexpected state '{}' for {}", other, instance_id),
        }
        if Instant::now() > deadline { bail!("{} did not reach COMPLETED in {}s", instance_id, timeout.as_secs()); }
...
async fn run_case(
    ...
    let instance_id = client.start_process(StartRequest {
        ...
        tenant_id: String::new(),
    }).await?.into_inner().process_instance_id;

    // Activate and complete the seed_data job.
    let deadline = Instant::now() + Duration::from_secs(10);
    let (job_key, claim_token, current_hash) = loop {
        let mut stream = client.activate_jobs(ActivateJobsRequest {
        ...
        if Instant::now() > deadline { bail!("seed_data job for {} not available", instance_id); }
    ...
    poll_completed(client, &instance_id, Duration::from_secs(20)).await
...
async fn main() -> Result<()> {
    ...
    let server_url = args.windows(2).find(|w| w[0] == "--server-url").map(|w| w[1].clone())
        .unwrap_or_else(|| "http://127.0.0.1:50071".to_string());
    let http_target_url = args.windows(2).find(|w| w[0] == "--http-target-url").map(|w| w[1].clone())
        .unwrap_or_else(|| "http://127.0.0.1:8080".to_string());

    println!("B9 Heterogeneous FFI proof");
    println!("  server:      {}", server_url);
    println!("  http-target: {}", http_target_url);

    wait_ready(&server_url, Duration::from_secs(15)).await.context("server not ready")?;
    ...
    let dmn_resp = client.register_dmn_decision(RegisterDmnDecisionRequest {
        dmn_source: DMN_SOURCE.to_string(),
        catalogue_toml: CATALOGUE_TOML.to_string(),
        input_fields: vec![
            FfiFieldSchemaProto { name: "score".to_string(), kind: "i64".to_string(), required: true },
        ],
        output_fields: vec![
            FfiFieldSchemaProto { name: "eligible".to_string(), kind: "bool".to_string(), required: false },
        ],
        tenant_id: String::new(),
        publisher: "heterogeneous_proof".to_string(),
    }).await.context("RegisterDmnDecision")?.into_inner();
    ...
    let http_resp = client.register_http_template(RegisterHttpTemplateRequest {
        ...
        input_fields: vec![
            FfiFieldSchemaProto { name: "client_id".to_string(), kind: "string".to_string(), required: true },
            FfiFieldSchemaProto { name: "amount".to_string(), kind: "i64".to_string(), required: true },
        ],
        output_fields: vec![
            FfiFieldSchemaProto { name: "score".to_string(), kind: "i64".to_string(), required: true },
            FfiFieldSchemaProto { name: "approved".to_string(), kind: "bool".to_string(), required: true },
        ],
        idempotency: "non_idempotent".to_string(),
        tenant_id: String::new(),
        publisher: "heterogeneous_proof".to_string(),
    }).await.context("RegisterHttpTemplate")?.into_inner();
    let http_template_id = http_resp.template_id_hex;
    println!("  http template_id: {}", &http_template_id[..16]);

    // 3. Compile the heterogeneous BPMN process.
    println!("  compiling heterogeneous BPMN...");
    let bpmn_xml = build_bpmn_xml(&http_template_id, &dmn_template_id);
    let compile_resp = client.compile(CompileRequest { bpmn_xml, validate_only: false })
        .await.context("Compile")?.into_inner();

    // Verify both bridge flags are present.
    let fst = &compile_resp.flag_symbol_table;
    let do_score_key = fst.iter().find(|(_, n)| *n == "do_score").map(|(k, _)| *k)
        .ok_or_else(|| anyhow!("do_score not in flag_symbol_table"))?;
    let do_eligible_key = fst.iter().find(|(_, n)| *n == "do_eligible").map(|(k, _)| *k)
        .ok_or_else(|| anyhow!("do_eligible not in flag_symbol_table"))?;
    println!("  do_score flag_key={}, do_eligible flag_key={}", do_score_key, do_eligible_key);

    let bytecode_version = compile_resp.bytecode_version;

    // 4. Run two cases.
    println!("  case 1: ACME → HTTP score=720 → dmn eligible=true → eligible_end");
    run_case(&mut client, bytecode_version.clone(), "ACME", "ACME").await?;

    println!("  case 2: REJECT → HTTP score=450 → dmn eligible=false → denied_end");
    run_case(&mut client, bytecode_version, "REJECT", "REJECT").await?;

    println!();
    println!("B9 Heterogeneous FFI proof PASSED");
    println!("  V&S Claim 2 substantiated: HTTP and dmn-lite vocabulary called from");
    println!("  the same BPMN process via Instr::ExecFfi through a shared Flag bridge.");
    Ok(())
}
