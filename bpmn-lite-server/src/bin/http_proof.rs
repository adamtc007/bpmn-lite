...
use bpmn_lite_server::grpc::proto::{
    ...
    HealthRequest, InspectRequest, RegisterHttpTemplateRequest, StartRequest,
};
...
async fn poll_completed(client: &mut BpmnLiteClient<Channel>, instance_id: &str, timeout: Duration) -> Result<()> {
    let deadline = Instant::now() + timeout;
    loop {
        let state = client.inspect(InspectRequest { process_instance_id: instance_id.to_string(), tenant_id: String::new() })
...
async fn run_case(
    ...
    let (job_key, claim_token, current_payload_hash) = loop {
        ...
        if Instant::now() > deadline { bail!("seed_inputs job for {} not available", instance_id); }
...
async fn main() -> Result<()> {
    ...
    println!("B7 HTTP FFI proof — server: {}  http-target: {}", server_url, http_target_url);
    ...
    let reg = client.register_http_template(RegisterHttpTemplateRequest {
    ...
    if !compile_resp.flag_symbol_table.iter().any(|(_, n)| n == "do_approved") {
    ...
    run_case(&mut client, bytecode_version.clone(), "ACME", "b7-approved", "ACME approved").await?;

    println!("  running case: REJECT (denied)...");
    run_case(&mut client, bytecode_version, "REJECT", "b7-rejected", "REJECT denied").await?;
