...
- **Q6 (opened 2026-07-22, during V5 boundary-timer lowering) — how does a boundary timer on a synchronous task escalate in v2?** Materially different from Q5/Gap 1: those were scope deferrals (v1 coverage genuinely absent live, safe to defer to v3). This is not — v1 *executes* boundary timers on `ExecNative`/`ExecFfi` today (`WaitArm::Internal{JOB_COMPLETE}` raced against a timer); v2's `V2RaceArm` has no equivalent arm (only `Timer`/`Msg`/`Effect`), and `V2Guard` carries no trigger mechanism of its own — deferring this would be a **regression**, not a scope-preserving choice, and boundary timers on service tasks are common BPMN, not a rare construct like inclusive gateways. Full detail, alternatives considered, and Adam's own lean (a guard-arming amendment — `GUARD>` gains a trigger spec, timer as the first kind, so a boundary timer is simply a guard scope wrapping arbitrary work) live in `EOP-PLAN-BPMN-ISA-002.md`'s V5.1 boundary-timer entry, not repeated here. **Retroactively resolves EX oracle §4 open item 1** ("exact `Command` shape for triggering an armed guard," deferred at V4 entry, never actually settled — `Command::V2TriggerGuard` exists but nothing in the word set showed what issues it). Held for ruling, not yet a §5 amendment.
