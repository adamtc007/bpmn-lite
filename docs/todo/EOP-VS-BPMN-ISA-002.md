...
**§4 interaction:** neither ruling requires a carve-out. E is a `Command`-shape change, not frame state. F's counter is store-side, in the same category as the lease/fence/journal — records *about* execution, never consulted *during* execution. §4 stands unamended.
