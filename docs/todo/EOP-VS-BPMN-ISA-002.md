...
- Q5: **Merged with Gap 1 (v0.9, §17) — one open question, not two.** Do any live EOP runbooks require dynamic-arity concurrency — parallel multi-instance *or* inclusive gateways — in v2? Both are the same problem (dynamic FORK arity) in different notation; both default-defer to v3 `FORK-DYN` absent a live requirement. Not answerable from this repository — for whoever owns the runbook inventory.
...
**Why this amendment exists:** a single V5-scoping session surfaced enough small, genuine loose ends — two reviewer questions dischargeable by evidence now on record, one dead lean now a built fact, one closed ruling that had to be reopened and corrected mid-session, exact figures that were previously approximate — that batching them into one consolidating amendment is cheaper than five separate ones, and matches how this document has always preferred to land related findings together (§13, §15). After this, the document is stable: no open reviewer question remains except the one genuinely external fact (Q5/Gap 1) no repository evidence can settle.
...
**Q5 and Gap 1 are the same question, merged in §10 above.** Both ask whether v2 needs dynamic-arity concurrency now; both default-defer to v3 absent a live requirement; neither is answerable from this repository.
...
**Scope of this amendment:** five small corrections and closures, batched. No K-invariant, ratified ruling, D3 integrity ring, or D4 theorem list changes. No new open question is introduced; one (Q5/Gap 1) is narrowed from two overlapping questions to one, and it is the sole reviewer question left unclosed in §10.
