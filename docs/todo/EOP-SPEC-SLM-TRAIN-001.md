...
**Status:** DRAFT v0.2 — blind review COMPLETE (17 findings: 3 blockers, 11 concerns, 3 nits; disposition table §S8). Blockers 1–3 remediated (finding 2 in code: `project_ir` + `DesignerDag::seed`, commit follows). **ONE OPEN RULING FOR ADAM (finding 5, §S5): board-vs-retrieved-subset listwise training lists.** Ratification after that ruling.
...
- **Objective:** listwise softmax cross-entropy including NOTA. **OPEN — ADAM'S RULING (finding 5):** DIR-002 A4 says lists are "over the board", but the ratified inference contract (§10.3/§10.6, `retrieved_subset_hash`) has tier-1 score the TIER-0 RETRIEVED SUBSET. Training-list shape must equal serving-list shape. **Recommendation: training lists = the real tier-0 retriever's K-subset over the board, with NOTA always appended** — same principle A4 invokes (train the inference shape), using the actual retriever so subset composition skew is impossible. Awaiting ruling; Phase B does not generate labels until ruled. Pointwise BCE as ablation only.
