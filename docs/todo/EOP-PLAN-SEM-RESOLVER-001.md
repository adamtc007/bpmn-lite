...
- **2.A BPMN authoring plane into SemOS.** Lift the 26 `op.*`/`prod.*` candidates into pack-schema verb declarations, a real (non-menu) `bpmn_dag` authoring graph, and constellation-map bindings; fix the map root using the generic `SlotKind` (the ob-poc seed still says `type: cbu` over a `bpmn_dags` table). Add dmn-lite decision candidates or record the exclusion (part of FK-F).
