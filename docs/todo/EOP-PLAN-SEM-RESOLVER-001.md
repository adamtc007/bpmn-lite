...
- **2.D ob-poc domain metadata (the long pole) — gate landed 2026-08-06 (ob-poc `b081126c`); backfill open.** Scoping receipt: 1,253 verbs; DAG-derivable universe 200+ transition verbs, 31 declaring `requires_states` (~184-verb backlog, mechanically derivable from DAG from-states); runtime enforces declared `requires_states` at `execute_verb_in_scope`, but only 37 slot-state mappings exist — backfill must stay within mapped slots or extend `slot_state_table.yaml` jointly (NoSlotMapping trap). Landed: `dag_transition_verbs_requires_states_drift_and_ratchet` — declared-vs-DAG drift refused (red→green on the live `cbu.submit-for-validation` inconsistency, fixed per the normative DAG), coverage floors ratchet-only. Phrase-collision linting already exists CI-enforced (`verbs atlas --lint-only`, zero-ERROR threshold) — item closed, EXCEPT 2 pre-existing same-domain collisions (`document.missing-for-entity` vs `document.list-missing`) need an owner ruling. Next: derive-and-backfill generator, pack-by-pack (lifecycle_resources is the 100% reference pack), floors raised with each tranche; promote the gate into `xtask reconcile` (catalogue.yml path-triggered CI) alongside the qualification-suite copy.
